...
<input type="hidden" name="model_id" id="task_model_id" value="">
...
?></span>
   ...
   <div class="mb_10">
   	<a href="http://www.wk.com/index.php?do=release" class="submit block">发布任务</a>
   </div>
...
var model_id = '';
                        ...
                        var url = window.location.href;
                          ...
                          if(is_rewrite>0){
 var new_url = url.replace('-model_id-'+model_id, '');
...
window.location.href = "index.php?r=task/search&province="+province+"&city="+city+"&area="+area;
}
   }
...
var new_url = '';
var is_rewrite = '0';
 $(".model_class").each(function(){//生成model_id数据 
                            if ($(this).is(":checked")) { 
                              model_id  += 'M'+$(this).attr("ext");
                            }
                        });
...
if(is_rewrite>0){
//判断是否存在model_id  
if(url.indexOf('model_id') ==-1){//不存在拼接
new_url = url; 
}else{//替换 
 new_url = url.replace("-model_id-"+model_id_val,"");     
}  

 new_url = new_url.replace(".html","-model_id-"+model_id+".html");
...
new_url = url; 
 ...
 new_url = new_url+'&model_id='+model_id; //新地址
...
location.href=new_url;//跳转 
  			...
  			var  model_id = $("#task_model_id").val(); 
