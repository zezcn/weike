<?php keke_tpl_class::checkrefresh('control/admin/tpl/admin_app_message_list', '1418787079' );?><!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
...
<div class="page_title">
    ...
    <div class="tool">
        <a href="index.php?do=<?php echo $do;?>&&view=<?php echo $view;?>" class="here">留言</a>
    </div>
</div>
           ...
           <tr>
             ...
             <tr>
                 <th>结果排序</th>
                  <td>
             		<select name="ord[]">
                           <option value="comment_id" <?php if($ord['0']=='comment_id' or !isset($ord['0'])) { ?> selected="selected"<?php } ?>><?php echo $_lang['default'];?><?php echo $_lang['order'];?></option>  	
                      </select>
                         <select name="ord[]">
...
<th>时间</th>
 ...
 <?php if(is_array($message_list)) { foreach($message_list as $key => $value) { ?>
               	<tr class="item">
               		<td><input type="checkbox" name="ckb[]" class="checkbox" value="<?php echo $value['comment_id'];?>"><?php echo $value['comment_id'];?></td>
<td><?php echo $value['username'];?></td>
<td title="<?php echo $value['content'];?>"><?php echo kekezu::cutstr($value['content'], 30) ?></td>
               		<td>
               			<?php if($value['on_time']){echo date('Y-m-d H:i:s',$value['on_time']); } ?>&nbsp;
</td>
<td>
<a class="button" href="index.php?do=<?php echo $do;?>&view=<?php echo $view;?>&op=del&pro_id=<?php echo $value['comment_id']?>" onclick="return cdel(this);"><span class="trash icon"></span><?php echo $_lang['delete'];?></a>
</td> 
</tr>
<?php } } ?>	
<tr>
